...
if ($http_code == 200 && isset($result['access_token'])) {
    ...
    try {
        ...
        // UPDATE: Fetch first_name, last_name, and status alongside customer_id
        $stmt = $pdo->prepare("SELECT customer_id, first_name, last_name, status FROM customers WHERE email = ?");
        $stmt->execute([$email]);
        ...
        if ($customer) {
            ...
            // SUCCESS: Return full profile for the Android SessionManager
            http_response_code(200);
            echo json_encode([
                "status"        => "success",
